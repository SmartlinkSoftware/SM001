...
public class DashboardProcessesPanel extends DashboardPanel {
    ...
    public void refresh() {
        try {
            ...
            Set<ProcessDefinition> pds = ProcessbaseApplication.getCurrent().getBpmModule().getProcessDefinitions();
            ...
            for (ProcessDefinition pd : pds) {
                Set<LightProcessInstance> pis = ProcessbaseApplication.getCurrent().getBpmModule().getLightProcessInstances(pd.getUUID());
                double countAll = 0;
                double countToday = 0;
