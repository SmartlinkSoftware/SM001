...
public class CustomXTeeConsumer extends CustomServiceGatewaySupport implements XTeeConsumer {
	protected final static org.apache.log4j.Logger LOGGER = LogManager.getLogger(CustomXTeeConsumer.class);
		
  private Map<String, XmlBeansXTeeMetadata> metadata;
  public static final String ROOT_NS = "ns5";

  public CustomXTeeConsumer(){
	  LOGGER.debug("Connector");
	  setWebServiceTemplate (new CustomServiceTemplate());
	  
  }
  
  @Override
  protected void initGateway() throws Exception {
	  LOGGER.debug("init gateway");
  ...
  private <I, O> XTeeMessage<O> sendRealRequest(XTeeMessage<I> input,
                                                ...
                                                CustomCallback callback,
                                                CustomExtractor extractor) throws XTeeServiceConsumptionException {
	  LOGGER.debug("send real request");
    XmlOptions options =
    ...
    try {
      ...
      WebServiceMessageCallback originalCallback =
          new StandardXTeeConsumerCallback(input.getContent(),
                                           ...
                                           getMarshaller(),
                                           curdata);
      WebServiceMessageExtractor originalExtractor = new CustomXTeeConsumerMessageExtractor(curdata);
